    ...
    <View style={styles.container}>
      ...
      {eliminadoActual && (
        <View style={styles.eliminadoBox}>
          ...
          {eliminados.length > 0 && (
            <Text style={styles.eliminadoTragos}>
              Posición #{eliminados.length} → {eliminados[eliminados.length - 1]?.tragos}
            </Text>
          )}
        </View>
      )}
      ...
      {terminado && jugadoresActivos.length === 1 && (
        <View style={styles.ganadorBox}>
          <Text style={styles.ganadorText}>🏆 {jugadoresActivos[0]} sobrevivió</Text>
          <Text style={styles.ganadorTragos}>¡Toma un vaso entero!</Text>
        </View>
      )}
      ...
      {eliminados.length > 0 && (
        <View style={styles.eliminadosList}>
          ...
          {eliminados.map((e, i) => (
            <Text key={i} style={styles.eliminadoItem}>
              #{e.posicion} {e.nombre} — {e.tragos}
            </Text>
          ))}
        </View>
      )}
      ...
      {!terminado ? (
        <TouchableOpacity
          style={[styles.btnGirar, girando && styles.btnDisabled]}
          onPress={girar}
          disabled={girando}
        >
